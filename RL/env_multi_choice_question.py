...
class MultiChoiceRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed, ask_large_fea_num=4, classify=True, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0,choice_num=2,fea_score="entropy"):
    ...
    def reset(self, embed=None):
        ...
        ###
        if self.classify:
            if user_like_random_fea not in self.kg.G['user'][self.user_id]['not_hate']:
    ...
    def _get_state(self):
        ...
        for item in self_cand_items:
            ...
            for fea in self.item_feature_pair[item]:
                ...
                i.append([idx[item2], idx[fea2]])###2
                i.append([idx[fea2], idx[item2]])###2
                v.append(1)
                v.append(1)
    ...
    def step(self, action, sorted_actions, embed=None):  
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            asked_feature = asked_feature[:self.choice_num]
    ...
    def _ask_update(self, asked_features):
        ...
        for asked_feature in asked_features:
            if asked_feature in self.feature_groundtrue:
                ...
                ###
                if self.classify:
                    if asked_feature not in self.kg.G['user'][self.user_id]['not_hate']:
                            self.user_special_acc_feature.append(asked_feature)
                ###
            else:
                ###
                if self.classify:
                    if asked_feature in self.kg.G['user'][self.user_id]['not_hate']:
