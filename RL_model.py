...
def train(args, kg, dataset, filename):
    ...
    env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, ask_large_fea_num=args.ask_large_fea_num, classify=args.classify, seed=args.seed, max_turn=args.max_turn, cand_num=args.cand_num, cand_item_num=args.cand_item_num,
    ...
    gcn_net = GraphEncoder(graph=G,device=args.device, entity=env.user_length+env.item_length+env.feature_length+1, emb_size=embed.size(1), kg=kg, classify=args.classify, embeddings=embed, \
...
def main():
    ...
    parser.add_argument('--classify', default=True, action='store_true')###
