...
class Agent(object):
    def __init__(self, device, memory, state_size, action_size, hidden_size, gcn_net, learning_rate, l2_norm, PADDING_ID, classify = False, EPS_START = 0.9, EPS_END = 0.1, EPS_DECAY = 0.0001, tau=0.01):
        ...
        self.policy_net = DQN(state_size, action_size, hidden_size, classify).to(device)
        self.target_net = DQN(state_size, action_size, hidden_size, classify).to(device)
