...
def evaluate(args, kg, dataset, filename):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, ask_large_fea_num=args.ask_large_fea_num, classify=args.classify, seed=args.seed, max_turn=args.max_turn,
    ...
    gcn_net = GraphEncoder(graph=G,device=args.device, entity=embed.size(0), emb_size=embed.size(1), kg=kg, classify=args.classify, embeddings=embed, \
    ...
    agent = Agent(device=args.device, memory=memory, state_size=args.hidden, action_size=embed.size(1), \
        hidden_size=args.hidden, gcn_net=gcn_net, learning_rate=args.learning_rate, l2_norm=args.l2_norm, PADDING_ID=embed.size(0)-1, classify=args.classify)
    ...
    # model_file = TMP_DIR[args.data_name] + '/RL-agent/' + filename + '-epoch-{}.pkl'.format(args.load_rl_epoch)
    # if not os.path.isdir(TMP_DIR[args.data_name] + '/RL-agent/'):
    #     os.makedirs(TMP_DIR[args.data_name] + '/RL-agent/')
...
def main():
    ...
    parser.add_argument('--ask_large_fea_num', type=int, default=4, help='the ask_large_fea_num')###
    parser.add_argument('--classify', default=False, action='store_true')###
    parser.add_argument('--load_rl_epoch', type=int, default=69, help='the epoch of loading RL model')
    ...
    for user in kg.G['user']:
        ...
        for item in kg.G['user'][user]['interact']:
            for fea in kg.G['item'][item]['belong_to']:
                not_hate.append(fea)
        not_hate = set(not_hate)
        kg.G['user'][user]['not_hate'] = not_hate
    ###
    #reset attr_num
    ...
    filename = 'train-data-{}-ask-large-num-{}-choice-num-{}-classify-{}-RL-cand_num-{}-cand_item_num-{}-embed-{}-seq-{}-gcn-{}'.format(
        args.data_name, args.ask_large_fea_num, args.choice_num, args.classify, args.cand_num, args.cand_item_num, args.embed, args.seq, args.gcn)
