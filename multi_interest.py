...
class SelfAttention(nn.Module):
    ...
    def __init__(self, d_hid, dropout=0.,anchor=False, classify=False):
        super().__init__()
        self.anchor=anchor
        if anchor:
    ...
    def forward(self, input_seq,anchor=None,s=None):
        ...
        if self.classify:
            input_seq = torch.cat((self.dropout(input_seq[:,:,:feature_dim-2]),input_seq[:,:,-2:]),dim=-1)  
        else:
...
class GraphEncoder(Module):
    def __init__(self,graph,device, entity, emb_size, kg, classify=False, embeddings=None, fix_emb=True, seq='rnn', gcn=True, hidden_size=100, layers=1, rnn_layer=1,u=None,v=None,f=None):
        ...
        self.num_pers=4
        self.multi_head_self_attention_init = nn.ModuleList([SelfAttention(self.hidden_size + self.classify*2, 0.3, classify=self.classify) for _ in range(self.num_pers)])
        self.multi_head_self_attention = nn.ModuleList([SelfAttention(self.hidden_size + self.classify*2, 0.3,anchor=True, classify=self.classify) for _ in range(self.num_pers)])
    def forward(self, b_state):
        ...
        for s, o in zip(b_state, batch_output):
            if self.classify:
                pos_tag = [[0,1] if fea - self.user_length - self.item_length in self.kg.G['user'][s['user']]['not_hate'] else [1,0] for fea in s['cur_node']]
                tensor_data = torch.tensor(pos_tag).unsqueeze(0).to(self.device)
                # pos_tag = [0.0 if fea - self.user_length - self.item_length in self.kg.G['user'][s['user']]['not_hate'] else 1.0 for fea in s['cur_node']]
                # tensor_data = torch.tensor(pos_tag).unsqueeze(-1).unsqueeze(0).to(self.device)
                tensor_original = (1-self.eps)*o[:len(s['cur_node']),:][None,:]+self.eps*gnn_embedding[s['cur_node']]
                tensor_merged = torch.cat([tensor_original, tensor_data], dim=-1)
                seq_embeddings.append(tensor_merged)
                # if sum(pos_tag) > 0:
                #     print('!!!')
            else:
        ...
        if rej_embedding!=None:
            ...
            if self.classify:
                # tensor_data = torch.ones(len(mm)).unsqueeze(-1).unsqueeze(-1).to(self.device)
                tensor_data = torch.tensor([0,1]).unsqueeze(0).unsqueeze(0).repeat(len(mm),1,1).to(self.device)
                mm = torch.cat([mm, tensor_data], dim=-1)
        ...
        for self_attention in self.multi_head_self_attention_init:
            ...
            seq_embedding.append(context.view(-1,1,self.hidden_size + self.classify*2))
            score.append(weight)
        ...
        for _ in range(1):
            ...
            for self_attention in self.multi_head_self_attention:
                ...
                seq_embedding[i]=context.view(-1,1,self.hidden_size + self.classify*2)
                score[i]=weight
                i+=1
